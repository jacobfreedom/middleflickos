...
func makeIconImage(size: CGFloat = 1024, background: NSColor = .white, foreground: NSColor = .black) -> NSImage {
...
guard args.count >= 2 else {
    fputs("Usage: generate_app_icon.swift /path/to/output.png\n", stderr)
    exit(2)
}
let outputPath = args[1]
let url = URL(fileURLWithPath: outputPath)

let image = makeIconImage(size: 1024, background: .white, foreground: .black)
do {
    try writePNG(image, to: url)
    print("Wrote 1024px base icon to: \(url.path)")
} catch {
    fputs("Error: \(error)\n", stderr)
    exit(1)
}
